...
#background = cv.imread('Scenario.JPG', cv.IMREAD_UNCHANGED)
background = cv.imread('Scenario.jpg', cv.IMREAD_UNCHANGED)
#Object = cv.imread('Paper.JPG', cv.IMREAD_UNCHANGED)
Object = cv.imread('Paper.jpg',cv.IMREAD_UNCHANGED)
object_w = Object.shape[1]
object_h = Object.shape[0]
result = cv.matchTemplate(background, Object, cv.TM_CCOEFF_NORMED)
print(result)
...
rectangle.append(rect)
rectangle, weights = cv.groupRectangles(rectangle, 1, 0.85)
print(rectangle)
if len(rectangle):
print('Found object!')
line_color = (0,255,0)
...
cv.rectangle(background,top_left,bottom_right, line_color, thickness, lin
e_type)
cv.putText(background, 'ToiletPaper', top_left, cv.FONT_HERSHEY_SIMPLEX,
0.7, (255,255,0), 1)
cv.imshow('Results', background)
cv.waitKey()
else:
print('Not Found!')
import cv2 as cv
...
import imutils
from vision import Vision
cap = cv.VideoCapture('Bldg79A.MP4')
...
rectangles = cascade_final.detectMultiScale(frame30, 1.1, 2,0,(10, 10),(100,1
00))
...
name = './Bldg79 38-48/frame' + str(currentframe) + '.jpg'
print ('Creating...' + name)
...
name = './79Neg/frame' + str(currentframe) + '.jpg'
print ('Creating...' + name)
# writing the extracted images
cv.imwrite(name, frame30)
# increasing counter so that it will
...
min_val, max_val, min_loc, max_loc = cv.minMaxLoc(result)
# The max location will contain the upper left corner pixel position for
the area
# that most closely matches our needle image. The max value gives an indi
cation
# of how similar that find is to the original needle, where 1 is perfect
and -1
# is exact opposite.
...
rectangles.append(rect)
rectangles, weights = cv.groupRectangles(rectangles, groupThreshold=1, ep
s=0.5)
...
top_left = (x, y)
bottom_right = (x + w, y + h)
cv.rectangle(haystack_img, top_left, bottom_right, line_color, lineTy
pe=line_type)
cv.putText(haystack_img, 'DAMAGE', top_left, cv.FONT_HERSHEY_SIMPLEX,
0.7, (0,0,255), 1)
cv.putText(haystack_img, 'Less than 40%', bottom_right, cv.FONT_HERSH
EY_SIMPLEX, 0.7, (0,255,255), 1)
return haystack_img
def draw_crosshairs(self, haystack_img, points):
marker_color = (255, 0, 255)
marker_type = cv.MARKER_CROSS
for (center_x, center_y) in points:
cv.drawMarker(haystack_img, (center_x, center_y), marker_color, marke
r_type)
return haystack_img
...
edge_filter.kernelSize = cv.getTrackbarPos('KernelSize', self.TRACKBAR_WI
NDOW)
edge_filter.erodeIter = cv.getTrackbarPos('ErodeIter', self.TRACKBAR_WIND
OW)
edge_filter.dilateIter = cv.getTrackbarPos('DilateIter', self.TRACKBAR_WI
NDOW)
...
kernel = np.ones((edge_filter.kernelSize, edge_filter.kernelSize), np.uin
t8)
eroded_image = cv.erode(original_image, kernel, iterations=edge_filter.er
odeIter)
dilated_image = cv.dilate(eroded_image, kernel, iterations=edge_filter.di
lateIter)
...
keypoints_needle, descriptors_needle = orb.detectAndCompute(self.needle_i
mg, None)
orb2 = cv.ORB_create(edgeThreshold=0, patchSize=patch_size, nfeatures=200
0)
keypoints_haystack, descriptors_haystack = orb2.detectAndCompute(original
_image, None)
FLANN_INDEX_LSH = 6
...
matches = flann.knnMatch(descriptors_needle, descriptors_haystack, k=
2)
except cv.error:
...
w = 1920
h = 1080
#hwnd = win32gui.FindWindow(None, 'Wheely 2 - Play Wheely 2 on Crazy Game
s - Google Chrome')
#hwnd = None
